...
DEFAULT_OUTPUT_DIR = Path.cwd() / "outputs" / "lerobot_dp_minimal"
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Minimal LeRobot DiffusionPolicy training loop using project policy adapter semantics."
    )
    ...
    parser.add_argument("--down-dims", default="512,1024,2048", help="Comma-separated UNet down dims")
    parser.add_argument("--num-train-timesteps", type=int, default=100)
    parser.add_argument("--num-inference-steps", type=int, default=None)
    parser.add_argument("--noise-scheduler-type", default="DDPM", choices=("DDPM", "DDIM"))
    parser.add_argument("--pretrained-backbone", default="none", choices=("none", "imagenet"))
    parser.add_argument("--save-every", type=int, default=0, help="Optional checkpoint cadence in steps")
    parser.add_argument("--wandb", action="store_true", help="Enable Weights & Biases logging")
    parser.add_argument("--wandb-project", default="lerobot-dp", help="W&B project name")
    ...
    parser.add_argument(
        "--normalization",
        default="min_max",
        choices=("min_max", "none"),
        help="Normalize state/action with dataset stats before DiffusionPolicy.forward.",
    )
    args = parser.parse_args()

    if args.steps <= 0:
        raise ValueError(f"--steps must be positive, got {args.steps}")
    down_dims = tuple(int(x.strip()) for x in args.down_dims.split(",") if x.strip())
    if args.horizon % (2 ** len(down_dims)) != 0:
        raise ValueError(f"--horizon must be divisible by {2 ** len(down_dims)} for down_dims={down_dims}")
    if args.n_action_steps > args.horizon - args.n_obs_steps + 1:
        raise ValueError("--n-action-steps must be <= horizon - n_obs_steps + 1")
    ...
    dataset_stats = _read_stats(dataset_root) if args.normalization == "min_max" else {}
    ...
    cfg = DiffusionConfig(
        ...
        use_group_norm=args.pretrained_backbone != "imagenet",
        down_dims=down_dims,
...
def _install_lerobot_namespace_shims(lerobot_src: Path) -> None:
    ...
    optimizers_mod.AdamConfig = AdamConfig
    ...
    class DiffuserSchedulerConfig(LRSchedulerConfig):
        def __init__(self, name: str = "cosine", num_warmup_steps: int = 500):
            self.name = name
            self.num_warmup_steps = num_warmup_steps

    schedulers_mod.LRSchedulerConfig = LRSchedulerConfig
    schedulers_mod.DiffuserSchedulerConfig = DiffuserSchedulerConfig
    ...
    shims = {
        ...
        "lerobot.policies.diffusion": lerobot_src / "lerobot" / "policies" / "diffusion",
    }
    ...
    torch.save(
        {
            ...
            "dp_config": _jsonable_config(cfg),
