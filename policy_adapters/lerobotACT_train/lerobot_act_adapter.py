...
class LeRobotACTAdapter:
    """Adapter for vanilla LeRobot ACT with current-project step-wise delta actions."""

    def __init__(self, cfg: LeRobotACTAdapterConfig | None = None):
        self.cfg = cfg or LeRobotACTAdapterConfig()
        if self.cfg.use_tactile:
            raise ValueError("vanilla LeRobot ACT adapter does not consume tactile fields")
    ...
    def spec(self) -> PolicyAdapterSpec:
        return PolicyAdapterSpec(
            ...
            tactile_used=False,
            notes=(
                "ACT n_obs_steps is 1 in this LeRobot implementation.",
                "action_delta_indices is range(chunk_size).",
                "Optional fixed front ROI is applied before adapter resize when configured.",
                "Tactile columns may remain in the dataset but must be excluded from input_features.",
            ),
        )
    ...
    def lerobot_input_features(self) -> dict[str, dict[str, Any]]:
        """Feature specs that vanilla LeRobot ACT should consume as inputs."""
    ...
    def lerobot_output_features(self) -> dict[str, dict[str, Any]]:
        """Feature specs that vanilla LeRobot ACT should predict."""
    ...
    def to_training_batch(self, batch: dict[str, Any], *, device: str | None = None) -> dict[str, Any]:
        """Convert a LeRobotDataset/DataLoader batch into the tensors ACTPolicy.forward expects.

        This is the training-side counterpart of :meth:`to_policy_input`. It deliberately keeps
        tactile fields out of the returned batch, resizes both cameras to the adapter image shape,
        and verifies that action chunks follow the configured ACT chunk size.
        """
        import torch
        ...
        front = _crop_batched_chw(front, self.cfg.front_roi)
    ...
    def normalize_training_batch(self, batch: dict[str, Any], stats: dict[str, Any]) -> dict[str, Any]:
        """Apply LeRobot-style mean/std normalization to state and action tensors."""
        out = dict(batch)
        out[STATE_KEY] = normalize_feature(out[STATE_KEY], stats, STATE_KEY)
        out[ACTION_KEY] = normalize_feature(out[ACTION_KEY], stats, ACTION_KEY)
        return out

    def normalize_policy_input(self, policy_input: dict[str, Any], stats: dict[str, Any]) -> dict[str, Any]:
        """Normalize online policy input with the same stats used during training."""
        out = dict(policy_input)
        out[STATE_KEY] = normalize_feature(out[STATE_KEY], stats, STATE_KEY)
        return out

    def unnormalize_action(self, action: Any, stats: dict[str, Any]) -> Any:
        """Convert normalized model output action back to raw relative action units."""
        return unnormalize_feature(action, stats, ACTION_KEY)
...
def normalize_feature(value: Any, stats: dict[str, Any], key: str, eps: float = 1e-8) -> Any:
    return _apply_mean_std(value, stats, key, inverse=False, eps=eps)


def unnormalize_feature(value: Any, stats: dict[str, Any], key: str, eps: float = 1e-8) -> Any:
    return _apply_mean_std(value, stats, key, inverse=True, eps=eps)


def _apply_mean_std(value: Any, stats: dict[str, Any], key: str, *, inverse: bool, eps: float) -> Any:
    if key not in stats:
        raise KeyError(f"missing normalization stats for {key!r}")
    entry = stats[key]
    if "mean" not in entry or "std" not in entry:
        raise KeyError(f"normalization stats for {key!r} must include mean and std")

    try:
        import torch

        if isinstance(value, torch.Tensor):
            mean = torch.as_tensor(entry["mean"], dtype=value.dtype, device=value.device)
            std = torch.as_tensor(entry["std"], dtype=value.dtype, device=value.device)
    ...
    while mean.ndim < arr.ndim:
        mean = np.expand_dims(mean, axis=0)
        std = np.expand_dims(std, axis=0)
    if inverse:
        return arr * std + mean
    return (arr - mean) / (std + np.float32(eps))
...
def _crop_batched_chw(value: Any, roi: ImageRoiConfig | None) -> Any:
    if roi is None:
        return value
