...
try:
    ...
    from .deploy_lerobot_act_runtime import LeRobotACTRuntime
    from .train_lerobot_act_minimal import _add_lerobot_to_path, _install_lerobot_namespace_shims
except ImportError:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Offline rollout-style ACT check: compare predicted raw actions with dataset GT actions "
            "and save curves/CSV metrics per episode."
        )
    )
    parser.add_argument("--checkpoint", required=True, help="Path to ACT checkpoint_last.pt")
    ...
    runtime = LeRobotACTRuntime.from_checkpoint(
        checkpoint_path,
...
def _save_plot(path: Path, pred: np.ndarray, gt: np.ndarray, err: np.ndarray, episode_index: int) -> None:
    ...
    fig.suptitle(f"LeRobot ACT offline rollout check - episode {episode_index}")
